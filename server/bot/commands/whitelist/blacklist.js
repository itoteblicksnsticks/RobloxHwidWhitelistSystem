...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('blacklist')
        .setDescription(`Removes the user's script access and blacklists them from being whitelisted in the future.`)
    ...
    async execute(interaction) {
        ...
        if (user_to_whitelist.blacklisted) {
            return interaction.reply({ embeds: [new MessageEmbed()
                .setColor(0x2F3136)
                .setTitle(' ')
                .setDescription(':x: This user is already blacklisted!')
                .setTimestamp()
                .setFooter({ text: ' ' })], ephemeral: true })
        }

        await user_to_whitelist.updateOne({ whitelisted: false, blacklisted: true })

        await interaction.options.getUser('user').send({
                embeds: [new MessageEmbed()
                    .setColor(0x2F3136)
                    .setTitle(' ')
                    .setDescription(`✅ You have been blacklisted by **${interaction.user}**!`)
                    .setTimestamp()
                    .setFooter({ text: ' ' })]
            })
        ...
        await interaction.reply({
            embeds: [new MessageEmbed()
                .setColor(0x2F3136)
                .setTitle(' ')
                .setDescription(`✅ Successfully blacklisted **${interaction.options.getUser('user')}**!`)
                .setTimestamp()
                .setFooter({ text: ' ' })]
        })
    }
